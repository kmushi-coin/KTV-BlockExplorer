...
// KTVParser handle
type KTVParser struct {
    *btc.BitcoinParser
    baseparser                         *bchain.BaseParser
    BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewKTVParser returns new KTVParser instance
func NewKTVParser(params *chaincfg.Params, c *btc.Configuration) *KTVParser {
    p := &KTVParser{
...
// GetChainParams contains network parameters for the main KTV network
...
func (p *KTVParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
    return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *KTVParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
    return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *KTVParser) ParseTx(b []byte) (*bchain.Tx, error) {
    t := wire.MsgTx{}
    r := bytes.NewReader(b)
...
func (p *KTVParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *KTVParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
    var tx bchain.Tx
...
func (p *KTVParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *KTVParser) IsAddrDescIndexable(addrDesc bchain.AddressDescriptor) bool {
...
func (p *KTVParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
    if len(tx.Vin) > input {
...
func (p *KTVParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
    if len(tx.Vin) > input {
        scriptHex := tx.Vin[input].ScriptSig.Hex
        if scriptHex != "" {
            script, _ := hex.DecodeString(scriptHex)
...
func (p *KTVParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
...
func (p *KTVParser) P2CSScriptToAddress(script []byte) ([]string, bool, error) {
    if len(script) < 50 {
